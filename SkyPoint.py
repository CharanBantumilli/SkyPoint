...
def smooth_move(current_x, current_y, width, height):
    # --- SPATIALNESS IMPROVEMENT ---
    # Calculate center of the hand's current "zone" in pixel space
    center_w = width / 2
    center_h = height / 2

    # Calculate offset from the center of the camera view
    offset_x = current_x - center_w
    offset_y = current_y - center_h

    # Map this offset to the screen, preserving the spatial relationship
    # This prevents the cursor from jumping to edges instantly
    screen_x = (SCREEN_W / 2) + (offset_x * 1.5) # 1.5 is spatial sensitivity
    screen_y = (SCREEN_H / 2) + (offset_y * 1.5)
    # -----------------------------
    ...
    if dist_moved > 80:
        adaptive_alpha = 0.8  # Fast movement: follow closely
    else:
        adaptive_alpha = SMOOTHING_FACTOR # Slow movement: smooth heavily
    ...
    # Apply Deadzone (Stop micro-movements smaller than 2 pixels)
    if abs(final_x - controller.prev_x) < 2 and abs(final_y - controller.prev_y) < 2:
        final_x, final_y = controller.prev_x, controller.prev_y
